...
type BadgeVariant  = 'up' | 'down' | 'warn' | 'alert'
    ...
    <div>
      ...
      <div className="bg-white border-b border-[#e2e6ec] px-8 h-[54px] flex items-center justify-between sticky top-0 z-40">
        ...
        <div className="flex items-center gap-4">
          <span className="text-[12px] text-[#8f9db0]">2026年4月13日（月）</span>
          {!hasReal && (
      ...
      <div className="p-8">
        ...
        <div className="flex gap-3 mb-7">
          ...
          <button className="px-4 py-2 bg-white border border-[#e2e6ec] hover:bg-slate-50 text-[#1a2332] text-[13px] font-semibold rounded-lg transition-colors shadow-sm">
            会計データを出力
          </button>
        </div>

        {/* KPI */}
        <div className="text-[11px] font-bold text-[#8f9db0] uppercase tracking-[0.8px] mb-3">経営サマリー — 2026年4月</div>
        <div className="grid grid-cols-4 gap-4 mb-7">
          <KpiCard label="今月の純利益（概算）" value={fmtK(netProfit)}   unit="千円" sub={`先月 1,645千円`}       badge="+12.4%" badgeVariant="up"    dividerColor="bg-blue-600"  />
          <KpiCard label="現預金残高"            value={fmtK(cashBalance)} unit="千円" sub="GMOあおぞら連携"       badge="+3.1%"  badgeVariant="up"    dividerColor="bg-green-500" />
          <KpiCard label="今月の売上"            value={fmtK(totalIncome)} unit="千円" sub={`目標 5,000千円 / ${Math.round(totalIncome/5000000*100)}%`} badge="目標未達" badgeVariant="warn" dividerColor="bg-amber-500" progress={Math.min(100, Math.round(totalIncome/5000000*100))} />
        ...
        <div className="grid grid-cols-[1fr_360px] gap-5">
            ...
            <div className="bg-white border border-[#e2e6ec] rounded-lg shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#e2e6ec]">
                <div>
                  ...
                  <div className="text-[11px] text-[#8f9db0] mt-0.5">OCR解析済み · 承認待ち</div>
                </div>
                ...
                <tbody>
                  {expenses.map((e: any, i: number) => (
                    <tr key={e.id ?? i} className="hover:bg-slate-50 border-b border-[#e2e6ec] last:border-0">
                      ...
                      <td className="px-5 py-2.5 text-[13px] text-[#8f9db0] whitespace-nowrap">
                        {String(e.expense_date).replace('2026-','').replace(/-/g,'/')}
                      </td>
                      <td className="px-5 py-2.5">
                        <span className="text-[10px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded">{e.source}</span>
                      </td>
                      <td className="px-5 py-2.5 text-[13px] font-semibold text-red-600 text-right font-mono">¥{Number(e.amount).toLocaleString()}</td>
                      <td className="px-5 py-2.5 text-center">
                        <StatusChip
                          variant={e.status === 'pending' ? 'pending' : 'approved'}
                          label={e.status === 'pending' ? '未承認' : '承認済'}
                        />
                      </td>
                    </tr>
                  ))}
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white border border-[#e2e6ec] rounded-lg shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#e2e6ec]">
                <div>
                  <div className="text-[13px] font-bold text-[#1a2332]">給与管理 — 2026年4月</div>
